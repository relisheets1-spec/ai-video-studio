 ...
 * /admin; для администратора студии нет — его почта принимает только код
 * администратора. Любая другая почта + код доступа, выданный в панели →
 * аккаунт (заводится при первом входе) и сессия студии на 30 дней. Писем нет.
 */
export async function POST(req: NextRequest) {
  ...
  // --- Администратор: только код администратора, студии у него нет ---
  if (isAdminEmail(email)) {
    if (checkAttempts(ip, "admin").blocked) {
