...
function rhythmRule(language: ContentLanguage): string {
  ...
  // Никакого «трети коротких предложений»: это правило рубило историю на обрубки.
  return (
    "8. РИТМ РЕЧИ: текст озвучит синтез речи, а длинные периоды с оборотами он читает с ложными паузами. " +
    "Ни одного предложения длиннее 22 слов; не больше одного придаточного и одного причастного или деепричастного оборота на предложение. " +
    "Длинные и короткие фразы чередуются естественно, а не механически. Короткое предложение продолжает мысль, а не начинает новую: " +
    "связность важнее ритма.\n" +
    "   ОБРАЗЕЦ: " +
    ex.good +
    "\n   ТАК НЕЛЬЗЯ: " +
    ex.bad +
...
export function buildBlueprintPrompt(opts: {
  ...
  const system =
    ...
    "\n\n" +
    (opts.plan.short
      ? "КОРОТКИЙ ФИЛЬМ: всего " +
        opts.plan.totalWords +
        " слов на " +
        opts.plan.minutes +
        " мин. Это ОДИН эпизод, а не три акта: один герой, одна ситуация, одно-два места, одна мысль. " +
        "Хук — первое предложение, развязка — последние два. Не пытайся уместить экспозицию, развитие и финал большой истории.\n\n"
    ...
    "ТРЕБОВАНИЯ К ПОЛЯМ:\n" +
    "- throughline: сквозной образ или вопрос, к которому рассказ вернётся " +
    (opts.plan.short ? "1-2 раза" : "3-4 раза") +
    ". Именно он делает историю цельной.\n" +
    ...
    "-" +
    opts.plan.beats.max +
    " битов по порядку. У каждого location — ОДНО конкретное место НА АНГЛИЙСКОМ (соседние биты могут делить место), " +
...
export function buildNarrationPrompt(opts: {
  ...
  const system =
    ...
    "» возвращается " +
    (plan.short ? "1-2 раза" : "3-4 раза") +
    " за историю — каждый раз в новом смысле, не дословно.\n" +
    ...
    "   СВЯЗНОСТЬ: каждое предложение вытекает из предыдущего — подхватывай образ, опирайся на местоимения и союзы; " +
    "никаких скачков между темами и предложений-обрубков, которые начинают новую мысль с нуля" +
    (plan.short
      ? ". Фильм короткий (~" +
        plan.askWords +
        " слов): это один цельный абзац одной истории, одна ситуация и одно место до границы бита, без новых героев после первой трети"
      : "") +
    ".\n" +
...
// ---------------------------------------------------------------------------
// Проход «объём» — только если текст заметно длиннее заказа
// ---------------------------------------------------------------------------

export function buildTrimPrompt(opts: { language: ContentLanguage; targetWords: number; currentWords: number }): {
  system: string;
  userPrefix: string;
} {
  const cut = opts.currentWords - opts.targetWords;
  const system =
    "Ты — редактор дикторского текста с маркерами |||.\n" +
    languageRule(opts.language) +
    "\nТЕКСТ ДЛИННЕЕ НУЖНОГО на " +
    cut +
    " слов. Сократи его ДО " +
    opts.targetWords +
    " слов (допуск ±5%): убирай второстепенные детали, повторы, украшения и лишние предложения, но не события и не имена.\n" +
    "Сохрани порядок, ВСЕ маркеры ||| ровно на тех же местах между теми же по смыслу предложениями, ритм коротких и длинных фраз.\n" +
    "Ничего не добавляй. Ответ — только текст с маркерами.";
  return { system, userPrefix: "ТЕКСТ (" + opts.currentWords + " слов):\n" };
}
...
export function buildRhythmRepairPrompt(opts: {
  ...
  const system =
    ...
    "ЗАДАЧА — ТОЛЬКО СЛИШКОМ ДЛИННЫЕ ПРЕДЛОЖЕНИЯ: каждое предложение длиннее 22 слов раздели на два-три по смыслу, не теряя фактов; " +
    "оборот в нём преврати в отдельное предложение с глаголом.\n" +
    "ОСТАЛЬНОЕ НЕ ТРОГАЙ: не укорачивай нормальные предложения ради «ритма», не дроби историю на обрубки, сохраняй связки между " +
    "предложениями — союзы, местоимения, подхват образа.\n" +
...
export function buildEditorPrompt(opts: { language: ContentLanguage; maxWords: number }): {
  system: string;
  userPrefix: string;
} {
  const base = buildEditorPromptBase(opts);
  // Связность важнее ритма — для фильмов любой длины.
  const keepFlow: Record<ContentLanguage, string> = {
    ru: "\nСВЯЗНОСТЬ: не дроби предложения без крайней нужды и сохраняй переходы между ними — союзы, местоимения, подхват образа. Связность важнее ритма.",
    kz: "\nБАЙЛАНЫС: сөйлемдерді қажетсіз бөлме, олардың арасындағы байланысты — жалғаулықтарды, есімдіктерді, бейненің жалғасын — сақта. Байланыс ырғақтан маңызды.",
    en: "\nFLOW: do not split sentences unless you must, and keep the transitions between them — conjunctions, pronouns, carried-over images. Flow matters more than rhythm.",
  };
  return { system: base.system + keepFlow[opts.language], userPrefix: base.userPrefix };
}

function buildEditorPromptBase(opts: { language: ContentLanguage; maxWords: number }): { system: string; userPrefix: string } {
  if (opts.language === "kz") {
    return {
      system:
