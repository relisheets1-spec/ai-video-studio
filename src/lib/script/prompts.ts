...
function rhythmRule(language: ContentLanguage, short = false): string {
  const ex = RHYTHM_EXAMPLES[language];
  if (short) {
    // На сотню слов правило «треть коротких предложений» рубит историю на обрубки.
    return (
      "8. РИТМ РЕЧИ: текст озвучит синтез. Ни одного предложения длиннее 20 слов, не больше одного придаточного и одного оборота на предложение. " +
      "Чередуй длинные и короткие фразы, но короткое предложение продолжает мысль, а не начинает новую.\n"
    );
  }
  return (
...
export function buildNarrationPrompt(opts: {
  ...
  const system =
    ...
    rhythmRule(opts.language, plan.short) +
    (plan.short
      ? "КОРОТКИЙ ФИЛЬМ: всего ~" +
        plan.askWords +
        " слов — это один цельный абзац одной истории, а не пересказ трёх актов. Каждое предложение вытекает из предыдущего: " +
        "подхватывай образ, опирайся на местоимения и союзы, оставайся в одной ситуации и одном месте до границы бита. " +
        "Никаких скачков между темами и новых героев после первой трети.\n"
      : "") +
...
export function buildEditorPrompt(opts: { language: ContentLanguage; maxWords: number; short?: boolean }): {
  system: string;
  userPrefix: string;
} {
  ...
  const keepFlow: Record<ContentLanguage, string> = {
    ru: "\nКОРОТКИЙ ТЕКСТ: не дроби предложения без крайней нужды и сохраняй переходы между ними — союзы, местоимения, подхват образа. Связность важнее ритма.",
    kz: "\nҚЫСҚА МӘТІН: сөйлемдерді қажетсіз бөлме, олардың арасындағы байланысты — жалғаулықтарды, есімдіктерді, бейненің жалғасын — сақта. Байланыс ырғақтан маңызды.",
    en: "\nSHORT TEXT: do not split sentences unless you must, and keep the transitions between them — conjunctions, pronouns, carried-over images. Flow matters more than rhythm.",
  };
