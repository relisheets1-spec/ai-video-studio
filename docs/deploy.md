...
Как устроено приложение целиком — `docs/PROJECT.md`. Здесь только сервер: всё,
что описано, лежит в репозитории и запускается как есть.
...
| Кадры и озвучка | `/var/lib/studio/media/films/<videoId>/scene_N.jpg\|mp3` (до 07.09.2026 — `png`) |
...
25 ГБ минус система ≈ 20 ГБ. Фильм на 15 минут — 30 кадров JPEG 95 % (~0,5 МБ)
плюс MP3 128 кбит/с (~14 МБ): около 30 МБ (старые фильмы с PNG — 70–80 МБ).
Это ~600 фильмов одновременно, а с уборкой через 30 дней предел практически
не достигается. Текущий расход видно в
